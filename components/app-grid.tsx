...
export default function AppGrid() {
  ...
  const fetchApps = async (page = 1, append = false) => {
    ...
    try {
      ...
      console.log("Fetching apps with params:", queryParams.toString())
    ...
    return (
      <div className="text-center py-16">
        <p className="text-muted-foreground">
          {categoryFilter
            ? `No apps found in the "${categoryFilter}" category${searchQuery ? ` matching "${searchQuery}"` : ""}.`
            : searchQuery
    ...
    <div>
      {dataSource && dataSource === "pipedream" && (
        <Alert className="mb-6">
          ...
          <AlertDescription>
            The app is currently using data from the Pipedream API because the Supabase database is empty.
          </AlertDescription>
        </Alert>
      )}
